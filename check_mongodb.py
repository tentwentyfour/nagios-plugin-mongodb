...
def main(argv):
    ...
    if action == "connections":
        ...
        return check_rep_lag(con, host_to_check, port_to_check, warning, critical, True, perf_data, max_lag, user, passwd, authdb, ssl, insecure, ssl_ca_cert_file, cert_file)
...
def check_rep_lag(con, host, port, warning, critical, percent, perf_data, max_lag, user, passwd, authdb="admin", ssl=None, insecure=None, ssl_ca_cert_file=None, cert_file=None):
    ...
    try:
        ...
        if serverVersion >= tuple("2.0.0".split(".")):
            ...
            if percent:
                err, con = mongo_connect(primary_node['name'].split(':')[0], int(primary_node['name'].split(':')[1]), ssl, user, passwd, None, authdb, insecure, ssl_ca_cert_file, cert_file)
                if err != 0:
                    return err
                primary_timediff = replication_get_time_diff(con)
